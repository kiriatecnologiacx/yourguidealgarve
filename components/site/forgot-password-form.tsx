...
export function ForgotPasswordForm({ locale }: { locale: Locale }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(t(locale, "auth.sendError"));
    } finally {
      setSubmitting(false);
    }
  }
    ...
    return (
      <div className="space-y-4">
        <div className="bg-green-50 border border-green-200 text-green-800 rounded-xl px-4 py-4 text-[13.5px]">
          <p className="font-semibold">{t(locale, "auth.emailSent")}</p>
          <p className="mt-1">
            {t(locale, "auth.emailSentHint")} <strong>{email}</strong> {t(locale, "auth.emailSentHint2")}
          </p>
          <p className="mt-2 text-[12px] text-green-700">{t(locale, "auth.checkSpam")}</p>
        </div>
